...
class turbine_base:
    ...
    def expand_to(self, Pout):
        """
        Adiabatically expand a fluid to pressure p_final
        , using a turbine with isentropic efficiency eta.
        """
...
class loop_turbine(turbine_base):
    ...
    def expand_to(self, Pout):
        ...
        for Tin in self.Tin_array:
            for Pin in self.Pin_array:
                turb = turbine_base(fluid = self.gas, efficiency = self.eta)
                turb.init_fluid(Tin = Tin, Pin = Pin, X = self.X)
                turb.expand_to(Pout = Pout)

                Work = turb.get_work()
                T_out = turb.get_T()
                deltaH = turb.get_deltaH()
                deltaS = turb.get_deltaS()
                deltaN = turb.get_deltaN()
